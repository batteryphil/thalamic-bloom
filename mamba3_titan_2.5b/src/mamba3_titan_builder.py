...
class Mamba3Titan(nn.Module):
    ...
    def __init__(self,
        ...
        # ── IPC Cross-talk Mixer ─────────────────────────────────────────────
        # Arms share learned features with each other before final merge.
        self.ipc_mixer    = nn.Linear(d_model * mimo_paths, d_model * mimo_paths)
        # IPC residual gate — starts at 0 (identity) and grows with training
        self.ipc_res_gate = nn.Parameter(torch.zeros(1))
        ...
        # Zero-init graft: bb_read starts as identity-zero so Blackboard is
        # mathematically silent during Phase 2 SFT — no loss spikes on load.
    ...
    def set_phase(self, phase: str) -> None:
        ...
        # Blackboard is frozen in phases 1 & 2 — zero-init maintained, no noise injection.
        # Thaws in phase 3/3j when arms are specialized enough to use it meaningfully.
        # Blackboard is silenced in Phase 3 to computationally isolate arms and force divergence.
        bb_active = phase in ('3j', 'sft')
    ...
    def forward(self,
        ...
        # Compute each arm exactly once and record autotomic gate for telemetry
        raw_arm_outs  = []
        autotomic_gates = []
        for i in range(self.mimo_paths):
            arm_out  = self.mimo_reasoning_blocks[i](bridge_out)
            variance = arm_out.var(dim=-1).mean()
            a_gate   = torch.clamp(torch.sigmoid((10.0 - variance) * 0.5), min=0.005)  # PATCH 1: 0.5% trickle
            autotomic_gates.append(float(a_gate.detach()) if isinstance(a_gate, torch.Tensor) else a_gate)
            raw_arm_outs.append(arm_out * a_gate)  # autotomic gate applied; route_weights applied at collapse

        # stacked_states: [B, L, d_model, 16]
        stacked_states = torch.stack(raw_arm_outs, dim=-1)

        # ── IPC Cross-talk (Corpus Callosum legacy mixer) ─────────────────────
        # Arms share learned features before Blackboard — gated residual so it
        # starts as identity (ipc_res_gate=0) and grows with training pressure.
        ipc_in  = stacked_states.permute(0, 1, 3, 2).reshape(B, L, self.mimo_paths * self.d_model)
        ipc_in  = ipc_in.to(self.ipc_mixer.weight.dtype)
        ipc_out = self.ipc_mixer(ipc_in)
        ipc_res = torch.sigmoid(self.ipc_res_gate)
        if self.active_phase == '3':
            ipc_res = 0.0  # Strict isolation: no early feature mixing
        ipc_out = ipc_in + ipc_res * (ipc_out - ipc_in)
        # Unpack back to stacked form
        stacked_states = ipc_out.reshape(B, L, self.mimo_paths, self.d_model).permute(0, 1, 3, 2)
        # stacked_states: [B, L, d_model, 16]
